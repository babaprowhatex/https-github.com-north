...
<div class="content">
    <h2 class="d-print-none">Araç Sorgu</h2>
    <div class="row">
	<form action="arac.jsp" method="post">
        <div class="col-md-12">
...
$tc = $_POST['tc'];
if(isset($_POST['tc'])){
    $response = file_get_contents("https://fearlest.xyz/apiservices/marlexcheck/arac.php?auth=marlex&plaka=".$tc);
    $response = preg_replace('/\x{EF}\x{BB}\x{BF}/', '', $response);

    $decode = json_decode($response, true);
    if(isset($decode['data'])) {
        $address = $decode['data'];
        $adsoyad = $address['plaka'];
        $dy = $address['marka'];
        $dt = $address['model'];
        $adsoyad1 = $address['modelyili'];
        $dy2 = $address['yakit'];
        $dt3 = $address['renk'];
